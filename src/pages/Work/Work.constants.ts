...
export const DESCRIPTIONS: any = [
  {
    ...
    //   "Developed testing tools in order to evaluate AI agent in-call performance and reproduce any failure casess",
    //   "Developed visualization dashboards to monitor company metrics",
    //   "Engineered prompts for AI call agent and transcript-to-JSON parsing for higher in-call performance and post-processing",
    //   "Added a bucketing system to store call recordings on Google Cloud while remaining HIPAA compliant.",
    //   "Integrated telephony API (Plivo) to reduce the highest automation costs by 20%",
    // ],
  ...
  },
  {
    ...
    description: [
      // "Investigated artificial pancreas algorithms for type 1 diabetes with the Doyle Group at Harvard SEAS.",
      // "Modeled real-time insulin absorption in patients for an insulin in-body sensor using the UVA/PADOVA type-1 diabetes simulator on MATLAB. It's purpose was to model insulin on board (IOB) on closed-loop insulin pumps to accurately calculate insulin sensitivity.",
      // "Cleaned fitness tracker data within (N=40) patients on MATLAB in order to classify specific exercise types (aerobic, anaerobic, and and interval) for use on closed-loop insulin pumps to adjust insulin delivery.",
